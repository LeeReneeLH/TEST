...
<script type="text/javascript">
	...
	function computeAmount(vId,vKbn){
		...
		var moneyCountDId = "moneyCountD_" + vId;		//数量(捆)-数量列1
		...
		if (typeof(goodsValue) != "undefined" && goodsValue != "") {
			amount =(moneyCountD * 1) * 1000 * goodsValue;
		}
	...
	function setSumDiff() {
		//取得一列的合计值
		var sumCountD = getSumValue('CountD');			//数量(捆)
...
<table  id="contentTable" class="table table-hover">
	<thead>
		<tr>
			...
			<!-- 未清分(捆) -->
			<th style="text-align:center;"><spring:message code="clear.public.quantity"/></th>
			<!-- 金额(元)-->
