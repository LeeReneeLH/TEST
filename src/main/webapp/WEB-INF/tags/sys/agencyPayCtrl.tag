...
<script type="text/javascript">
	...
	function computeAmount(vId,vKbn){
		...
		var moneyCountWId = "moneyCountW_" + vId;		//完整(捆)-数量列1
		var moneyCountCId = "moneyCountC_" + vId;		//残损(捆)-数量列2
		/* var moneyCountYId = "moneyCountY_" + vId;		//已清分(捆)-数量列3 */
		var moneyCountAId = "moneyCountA_" + vId;		//ATM(捆)-数量列4
		var goodsValueId = "parValue_" + vId;			//实际面值
		//值的取得
		var moneyCountW = $("#" + moneyCountWId).val();
		var moneyCountC = $("#" + moneyCountCId).val();
		/* var moneyCountY = $("#" + moneyCountYId).val(); */
		var moneyCountA = $("#" + moneyCountAId).val();
		var goodsValue = $("#" + goodsValueId).val();
		
		//面值对应的捆数未输入的场合，退出
		if ((moneyCountW == undefined || moneyCountW == "") && (moneyCountC == undefined || moneyCountC == "") && (moneyCountA == undefined || moneyCountA == "")) {
			$("#" + toId).val("");
			$("#" + toId + "Show").html("");
		...
		//完整(捆)
		if (moneyCountW == undefined || moneyCountW == "") {
			moneyCountW = "0";
		}
		//残损(捆)
		if (moneyCountC == undefined || moneyCountC == "") {
			moneyCountC = "0";
		}
		//已清分(捆)
		...
		if (typeof(goodsValue) != "undefined" && goodsValue != "") {
			amount =(moneyCountW * 1 + moneyCountC*1 + moneyCountA * 1)* 1000* goodsValue;	
		}
	...
	function setSumDiff() {
		//取得一列的合计值
		var sumCountW = getSumValue('CountW');			//完整(捆)
		var sumCountC = getSumValue('CountC');			//残次(捆)
		/* var sumCountY = getSumValue('CountY');			//已清分(捆) */
		var sumCountA = getSumValue('CountA');			//ATM(捆)
		var sumOrderAmount = getSumValue('Amount');		//金额(元)
		//合计值的设定
		$("#SumCountW").html(formatCurrency(sumCountW));
		$("#SumCountC").html(formatCurrency(sumCountC));
...
<table  id="contentTable" class="table table-hover">
	<thead>
		<tr>
			...
			<!-- 完整券(捆) -->
			<th style="text-align:center;"><spring:message code="clear.public.Wzq"/>(<spring:message code="clear.public.bundle"/>)</th>
			<!-- 残损券(捆) -->
			<th style="text-align:center;"><spring:message code="clear.public.Csq"/>(<spring:message code="clear.public.bundle"/>)</th>
			<!-- 已清分(捆) -->
	...
	<tbody>
			...
			<tr>
				...
				<!-- 完整（捆） -->
					<td style="text-align:right;width:100px;">
						<form:input id="moneyCountW_${status.index}" path="${bindListName}[${status.index}].columnValue1" 
							...
							onkeydown="changeEnter(event,'moneyCountW_','${status.index}')" 
							UseType="CountW"
							readOnly="${vReadOnly}"
				...
				<!-- 残损（捆） -->
					<td style="text-align:right;width:100px;">
						<form:input id="moneyCountC_${status.index}" path="${bindListName}[${status.index}].columnValue2" 
							onkeyup="value=value.replace(/[^0-9]/g,'');computeAmount('${status.index}',true)" 
							onchange="value=value.replace(/[^0-9]/g,'');computeAmount('${status.index}',true)"
							onkeydown="changeEnter(event,'moneyCountC_','${status.index}')" 
							UseType="CountC"
							readOnly="${vReadOnly}"
							maxlength="3" class="digits" style="width:90px;text-align:right;ime-mode:disabled;"
							tabindex="1" /></td>
				<!-- 已清分（捆） -->
				<%-- 	<td style="text-align:right;width:100px;">
						<form:input id="moneyCountY_${status.index}" path="${bindListName}[${status.index}].columnValue3" 
							onkeyup="value=value.replace(/[^0-9]/g,'');computeAmount('${status.index}',true)" 
							onchange="value=value.replace(/[^0-9]/g,'');computeAmount('${status.index}',true)"
				...
				<!-- ATM（捆） -->
					<td style="text-align:right;width:100px;">
						<form:input id="moneyCountA_${status.index}" path="${bindListName}[${status.index}].columnValue4" 
							onkeyup="value=value.replace(/[^0-9]/g,'');computeAmount('${status.index}',true)" 
							onchange="value=value.replace(/[^0-9]/g,'');computeAmount('${status.index}',true)"
		...
		<tr>
			...
			<td style="text-align:right;">
				<label id="SumCountW"></label>
			</td>
			<td style="text-align:right;">
				<label id="SumCountC"></label>
			</td>
			<!-- <td style="text-align:right;">
