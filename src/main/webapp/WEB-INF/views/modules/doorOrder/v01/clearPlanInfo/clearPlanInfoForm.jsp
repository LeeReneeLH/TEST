...
<body>
	<ul class="nav nav-tabs">
		...
		<%-- gzd 2020-04-14  修改链接  ${ctx}/doorOrder/v01/clearPlanInfo/form?id=${clearPlanInfo.id} --%>
		<li class="active"><a>
			<c:choose>
				<c:when test="${not empty clearPlanInfo.id}">
					<spring:message code="door.clearPlan.edit" /> 
				</c:when>
				<c:otherwise>
					<spring:message code="door.clearPlan.form" /> 
				</c:otherwise>
			</c:choose>
		</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="clearPlanInfo" action="${ctx}/doorOrder/v01/clearPlanInfo/saveTemp" method="post" class="form-horizontal">
		...
		<form:hidden path="id"/>
			<div class="control-group" style="margin-left: 80px;">
				...
				<div style="margin-left: 100px; margin-top: -20px">
					<form:select path="clearPlanUserDetailList" items="${sto:getUsersByTypeAndOffice('40',fns:getUser().office.id)}" itemLabel="escortName" itemValue="id" htmlEscape="false" class="input-xxlarge required"/>
					<span class="help-inline"><font color="red">*</font> </span>
				</div>
				<form:hidden path="clearManName" />
			</div>
			<div class="control-group">
				<label class="control-label">机具：</label>
				<div class="controls">
					<form:select path="clearPlanInfoList" items="${clearPlanList}" itemLabel="equipmentName" itemValue="id" htmlEscape="false" class="input-xxlarge required"/>
					<span class="help-inline"><font color="red">*</font> </span>
				</div>
			</div>
