...
<head>
	<!-- 商行取款列表-->
	<title><spring:message code="clear.bankGet.title"/></title>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$("#btnSubmit").click(function(){
				$("#searchForm").attr("action", "${ctx}/clear/v03/bankGetCtrl/list");
				$("#searchForm").submit();
			});
		});
		...
		function authorize(inNo) {
			...
			var url = "${ctx}/clear/v03/bankGetCtrl/reverse";
			var refreshUrl = "${ctx}/clear/v03/bankGetCtrl/reverseList";
			var ctx  = "${ctx}";
		...
		function displayAuthorizeReason(outNo) {
			var content = "iframe:${ctx}/clear/v03/bankGetCtrl/displayAuthorizeReason?outNo="+outNo;
			top.$.jBox.open(
					content,
					// 冲正原因
...
<body>
	<ul class="nav nav-tabs">
		<!-- 商行取款列表 -->
		<li class="active"><a href="${ctx}/clear/v03/bankGetCtrl/list"><spring:message code="clear.bankGet.title" /></a></li>
		<shiro:hasPermission name="clear:bankGetCtrl:add">
			<!-- 商行取款登记 -->
			<li><a href="${ctx}/clear/v03/bankGetCtrl/form"><spring:message code="clear.bankGet.register" /></a></li>
		</shiro:hasPermission>
	</ul>
	<div class="row">
	...
	<table id="contentTable" class="table table-hover">
		...
		<tbody>
		<c:forEach items="${page.list}" var="clOutMain" varStatus="statusIndex">
			<tr>
				<!-- 入库单号 -->
				<td><a href="${ctx}/clear/v03/bankGetCtrl/view?outNo=${clOutMain.outNo}">
					${clOutMain.outNo}
				</a></td>
				<!-- 客户名称 -->
				<td>
					${clOutMain.rOffice.name}
				</td>
				<!-- 登记金额 -->
				...
				<span style='width:30px;display:inline-block;'> 
					<shiro:hasPermission name="clear:bankGetCtrl:reverse">
						<c:if test="${'1' eq clOutMain.status}">
							<!-- 冲正 -->
							<a id="aReverse" href="#" onclick="authorize('${clOutMain.outNo}');" title="<spring:message code='common.reverse'/>">
