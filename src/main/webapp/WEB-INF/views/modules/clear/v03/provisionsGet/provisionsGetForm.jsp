...
<head>
	...
	function changeAmountToBig(amount, toId) {
		...
		$.ajax({
			url : ctx + '/clear/v03/provisionsOut/changRMBAmountToBig?amount=' + amount,
			type : 'post',
			dataType : 'json',
...
<body>
	<ul class="nav nav-tabs">
		<!-- 备付金取回列表 -->
		<li><a href="${ctx}/clear/v03/provisionsOut/list"><spring:message code="clear.provisionOut.list" /></a></li>
		<!-- 备付金取回登记 -->
		<li class="active"><a href="${ctx}/clear/v03/provisionsOut/form" ><spring:message code="clear.provisionOut.register" /></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="clOutMain" onkeydown="if(event.keyCode==13)return false;" action="${ctx}/clear/v03/provisionsOut/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="row">
			...
			<!-- 面值明细 -->
			<div class="span6">
				<sys:provisionsIn dataSource="${clOutMain.denominationList}"  bindListName="denominationList" totalAmountId="outAmount"/>
			</div>
		</div>
