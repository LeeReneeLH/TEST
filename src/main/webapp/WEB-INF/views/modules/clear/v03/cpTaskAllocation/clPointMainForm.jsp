...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			checkSession();
			$("#clearGroupDiv").load("${ctx}/clear/v03/cpTaskAllocation/getClearGroup?clearGroupId="+$('#clearGroup').val());
		...
		function getClearGroup(type) {
			checkSession();
			$("#clearGroupDiv").load(
					"${ctx}/clear/v03/cpTaskAllocation/getClearGroup?clearGroupId="
							+ type);
		}
		// 单独分配各人员数量
		...
		function checkTaskCount(){
			...
			if (planType==1) {
				$.ajax({
					url : "${ctx}/clear/v03/cpTaskAllocation/checkTaskCount",
					type : 'POST',
					async: false,
					dataType : 'json',
					data : {
		...
		function pastCheck(){
			...
			var url = "${ctx}/clear/v03/cpTaskAllocation/save?userCount="+userCount;
			var totalsum = document.getElementById("totalsum").innerText;
			if(totalsum == 'NaN'){
				alertx("分配数量有误，请重新输入");
				return;
...
<body>
	<ul class="nav nav-tabs">
		<!-- 任务分配列表 -->
		<li><a href="${ctx}/clear/v03/cpTaskAllocation/"><spring:message code='clear.task.distribution.list' /></a></li>
		<!-- 任务分配 -->
		<li class="active"><a href="${ctx}/clear/v03/cpTaskAllocation/form"><spring:message code='clear.task.distribution.register'/></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="clTaskMain" action=""
							...
							<table id="contentListTable"
								...
								<tbody>
									<tr>
										...
										<%-- <c:forEach items="${sto:getGoodDictListWithFg('cnypden')}"
											var="item">
											<c:if test="${clTaskMain.denomination eq item.label}">
												<td style="text-align: center;">${clTaskMain.totalCount}（捆）</td>
											</c:if>
											<c:if test="${!(clTaskMain.denomination eq item.label)}">
										...
										<c:forEach items="${bankPayInfo}" var="bankPay">
											<td style="text-align: center;">${bankPay.countPeopleBank}</td>
										</c:forEach>
									</tr>
		...
		<div class="form-actions" style="clear:both;width:100%;">
			...
			<input id="btnCancel" class="btn" type="button" value="<spring:message code='common.return'/>"
				onclick="window.location.href='${ctx}/clear/v03/cpTaskAllocation/back'" />
		</div>
