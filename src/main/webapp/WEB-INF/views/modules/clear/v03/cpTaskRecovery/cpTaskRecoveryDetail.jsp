...
<body>
	<ul class="nav nav-tabs">
		<!-- 任务回收列表 -->
		<li><a href="${ctx}/clear/v03/cpTaskRecovery/"><spring:message code='clear.task.recovery.list'/></a></li>
		<!-- 任务回收明细 -->
		<li class="active"><a href="#" onclick="javascript:return false;"><spring:message code='clear.task.recovery.detail'/></a></li>
	</ul><br/>
	<form:form id="detailForm" modelAttribute="cpTaskMain" action=""
				...
				<div class="control-group item">
					...
					<div class="controls" style="margin-left: 185px;">
						<input type="text" id="busType" name="busType"
							value="${fns:getDictLabel(cpTaskMain.busType,'clear_businesstype','')}"
							style="width: 170px; text-align: right;" readOnly="readOnly" />
					</div>
				</div>
				<div class="clear"></div>
				...
				<div class="control-group item">
					...
					<div class="controls" style="margin-left: 185px;">
						<input type="text" id="planType" name="planType"
							value="${fns:getDictLabel(cpTaskMain.planType,'clear_plan_type','')}"
							style="width: 170px; text-align: right;" readOnly="readOnly" />
					</div>
				</div>
			</div>
				...
				<div class="control-group item">
					...
					<div class="controls" style="margin-left: 185px;">
						<input type="text" id="taskType" name="taskType"
							value="${fns:getDictLabel(cpTaskMain.taskType,'clear_task_type','')}"
							class="input-medium" style="width: 170px; text-align: right;"
							readOnly="readOnly" />
					</div>
				</div>
				<div class="control-group item">
					...
					<div class="controls" style="margin-left: 185px;">
							<input type="text" id="totalAmt"
								value="<fmt:formatNumber value="${cpTaskMain.totalAmt}" 
								pattern="#,##0.00#" />"
				...
				<div class="control-group item">
					...
					<div class="controls" style="margin-left: 185px;">
						<input type="text" id="goodsId" name="goodsId"
							value="${sto:getGoodsName(cpTaskMain.goodsId)}"
			  ...
			  <div class="span20" style="text-align: right">
			    ...
			    <div style="overflow-y: auto; height: 515px;">
					<table id="contentTable" class="table table-hover">
						...
						<tbody>
							<c:forEach items="${cpTaskMain.clTaskDetailList}" var="clDetail" varStatus="status">
								<!-- 任务详细 -->
								<tr>
									...
									<c:forEach items="${sto:getGoodDictListWithFg('cnypden')}" var="item">
										<c:choose>
											<c:when test="${cpTaskMain.denomination eq item.value}">
												<td style="text-align: center">${clDetail.totalCount}</td>
											</c:when>
											<c:otherwise>
						...
						<tr>
							...
							<th style="text-align: center"></th>
							<th id="totalsum" style="text-align: center">${cpTaskMain.totalCount}</th>
							<c:forEach items="${sto:getGoodDictListWithFg('cnypden')}" var="item">
								<c:choose>
									<c:when test="${cpTaskMain.denomination eq item.value}">
										<th style="text-align: center">${cpTaskMain.totalCount}</th>
									</c:when>
									<c:otherwise>
