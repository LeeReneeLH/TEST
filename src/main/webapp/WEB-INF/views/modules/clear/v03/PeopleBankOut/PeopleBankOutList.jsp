...
<head>
	<!-- 从人行复点出库列表 -->
	<title><spring:message code='clear.peopleBankOut.title'/></title>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$("#btnSubmit").click(function(){
				$("#searchForm").attr("action", "${ctx}/clear/v03/peopleBankOut/list");
				$("#searchForm").submit();
			});
		});
		// 打印明细
		function printDetail(inNo){
			var content = "iframe:${ctx}/clear/v03/peopleBankOut/printPeopleBankOut?inNo=" + inNo;
			top.$.jBox.open(
					content,
					//打印
		...
		function pastCheck(inNo){
			var url = "${ctx}/clear/v03/peopleBankOut/reverse";
			var refreshUrl = "${ctx}/clear/v03/peopleBankOut/reverseList";
			var ctx  = "${ctx}";
		...
		function displayAuthorizeReason(inNo) {
			var content = "iframe:${ctx}/clear/v03/peopleBankOut/displayAuthorizeReason?inNo="+inNo;
			top.$.jBox.open(
					content,
					// 冲正原因
		...
		function checkTellerAccounts(inNo){
			var boo=true;
			$.ajax({
				url : "${ctx}/clear/v03/peopleBankOut/checkTellerAccounts",
				type : 'POST',
				async: false,
				dataType : 'json',
				data : {
		...
		function checkReverseCount(inNo){
			var boo=true;
			$.ajax({
				url : "${ctx}/clear/v03/peopleBankOut/checkReverseCount",
				type : 'POST',
				async: false,
				dataType : 'json',
				data : {
...
<body>
	<ul class="nav nav-tabs">
		<!-- 从人行复点出库列表 -->
		<li class="active"><a href="${ctx}/clear/v03/peopleBankOut/list"><spring:message code='clear.peopleBankOut.title'/></a></li>
		<shiro:hasPermission name="clear:peopleBankOut:add">
			<!-- 从人行复点出库登记 -->
			<li><a href="${ctx}/clear/v03/peopleBankOut/form"><spring:message code='clear.peopleBankOut.register'/></a></li>
		</shiro:hasPermission>
	</ul>
	<div class="row">
	...
	<table  id="contentTable" class="table table-hover">
		...
		<tbody>
			<c:forEach items="${page.list}" var="allocate" varStatus="statusIndex">
				<tr>
					<!-- 入库单号 -->
					<td><a href="${ctx}/clear/v03/peopleBankOut/view?inNo=${allocate.inNo}">
						${allocate.inNo}
					</a></td>
					<!-- 登记金额 -->
					...
					<span style='width:35px;display:inline-block;'> 
						<shiro:hasPermission name="clear:peopleBankOut:reverse">
							<!-- 冲正 -->
					...
					<span style='width:25px;display:inline-block;'> 
						<shiro:hasPermission name="clear:peopleBankOut:print">
							<!-- 打印 -->
