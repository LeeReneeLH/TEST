...
<head>
	...
	<script type="text/javascript">
		...
		// 加载数据动态加载tr
		function loadRow(list, idx, tpl, row){
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="#"><spring:message code="label.atm.add.manuallyGenerated"/></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="atmPlanInfo" action="${ctx}/atm/v01/atmPlanInfo/editSave" method="post" class="form-horizontal" onsubmit="checkDisable()">
		<%-- <form:hidden path="id"/> --%>
				...
				<div class="controls">
					...
					<script type="text/template" id="addPlanTpl">//<!--
						<tr id="addPlanList{{idx}}">
							...
							<td>
								<input id="addPlanList{{idx}}_atmAccount" name="addPlanList[{{idx}}].atmAccount" readonly="readonly" type="text" value="{{row.atmAccount}}" maxlength="60" class="input-small"/>
								<span class="help-inline"><font color="red">*</font> </span>
							</td>
							<td>
								<input id="addPlanList{{idx}}_atmAddress" name="addPlanList[{{idx}}].atmAddress" readonly="readonly" type="text" value="{{row.atmAddress}}" maxlength="60" class="input-small "/>
							</td>
							<td>
								<input id="addPlanList{{idx}}_addAmountStr" name="addPlanList[{{idx}}].addAmountStr" type="text" maxlength="5" value="{{row.addAmount}}" maxlength="60" class="input-small required digits"/>
							</td>
							<td>
								<input id="addPlanList{{idx}}_getBoxNumStr" name="addPlanList[{{idx}}].getBoxNumStr" type="text" maxlength="3" value="{{row.getBoxNum}}" maxlength="60" class="input-small required digits"/>
							</td>
							<td>
					...
					<script type="text/javascript">
						...
						$(document).ready(function() {
							...
							for (var i=0; i<data.length; i++){
								loadRow('#addPlanList', addPlanRowIdx, addPlanTpl, data[i]);
