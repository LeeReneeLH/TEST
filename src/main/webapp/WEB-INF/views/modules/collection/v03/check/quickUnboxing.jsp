...
<head>
	...
	<script type="text/javascript">
		...
		function save() {
			//是否存在差额
			if(changeAmountToNum($('#diffAmount').val())!=0 && Math.abs(changeAmountToNum($('#diffAmount').val())) > 1){
			...
			}else if(changeAmountToNum($('#diffAmount').val())!=0 && Math.abs(changeAmountToNum($('#diffAmount').val())) <= 1){
...
<body>
	...
	<form:form id="inputForm" modelAttribute="checkCashMain" action="${ctx}/collection/v03/checkCash/save" method="post" class="form-horizontal">
		...
		<c:set var="col1" value="100px"/>
		<c:set var="col2" value="190px"/>
		<c:set var="col3" value="190px"/>
		...
		<div class="row">
			<%-- 页面左侧 --%>
			<!-- <div id="jbox-content" class="jbox-content" style="height:auto;overflow:hidden;overflow-y:auto;">
				<div style="margin:10px;min-height:30px;height:auto;padding-left:40px;text-align:left;">
					<span class="jbox-icon jbox-icon-info" style="position: absolute; top: 55px; left: 15px; width: 32px; height: 32px;">
					</span>是否进行快速拆箱
				</div>
			</div> -->
			<div class="span7" >
				<div class="control-group" style="margin-top: -2px;margin-left: 40px;">
					<i style="position: absolute; top: 12px; left: 15px; width: 32px; height: 32px; color:#FF9800;" class="fa fa-warning fa-2x"></i>是否确认拆箱
				</div>
			...
			<%-- 页面中部 --%>
			<div class="span5" style="margin-left: 80px;display:none">
				<%-- 纸币 --%>
