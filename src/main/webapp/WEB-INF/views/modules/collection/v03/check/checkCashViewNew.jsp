...
<body>
	<ul class="nav nav-tabs">
		<!-- 款箱拆箱列表 -->
		<li id="displayLi1"><a onClick="window.top.clickSelect();" href="${ctx}/collection/v03/checkCash/"><spring:message code="door.checkCash.title" /><spring:message code="common.list" /></a></li>
		<!-- 历史拆箱列表 -->
		<li id="displayLi2"><a onClick="window.top.clickSelect();" href="${ctx}/collection/v03/checkCash/historyList"><spring:message code="clear.atmCashBox.history" /></a></li>
		<!-- 款箱拆箱登记(修改) -->
		<li id="displayLi3" class="active">
				...
				<shiro:hasPermission name="check:checkCash:edit">
					<c:choose>
						<c:when test="${not empty checkCashMain.outNo}">
							<a>
								<spring:message code="door.checkCash.title" /><spring:message code="common.view" /> 
							</a>
						</c:when>
						<c:otherwise>
							<a>
