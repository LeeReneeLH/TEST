...
<head>
	...
	<script type="text/javascript">
		...
		function urlLinkToMenu(href){
			var urlStart = href.indexOf("/frame");
			var urlEnd = href.lastIndexOf("/");
...
<body>
	...
	<form id="searchForm" action="${ctx}/act/task/process/" method="post" class="breadcrumb form-search">
		<select id="category" name="category" class="input-medium">
			...
			<c:forEach items="${fns:getDictList('act_start_authority')}" var="dict">
	...
	<table class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			<c:forEach items="${page.list}" var="object">
				...
				<tr>
					<td>${fns:getDictLabel(process.category,'act_start_authority','无分类')}</td>
					...
					<td>
						<a onclick="urlLinkToMenu('${object[2]}')" href="${ctx}/act/task/form?procDefId=${process.id}&procDefKey=${process.key}">启动流程</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
