...
<body>
	...
	<form:form id="inputForm" modelAttribute="role" action="${ctx}/sys/role/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<form:hidden path="office.id" value="${fns:getConfig('top.office.id')}"/>
		...
		<div class="form-actions">
			...
			<c:choose>
				<c:when test="${role.createBy.id ne '1'}">
					<input id="btnSubmit" class="btn btn-primary" type="submit" value="保存"/>&nbsp;
				</c:when>
				<c:otherwise>
					<c:if test="${fns:getUser().userType eq '0'}">
					<input id="btnSubmit" class="btn btn-primary" type="submit" value="保存'/>"/>&nbsp;
					</c:if>
				</c:otherwise>
			</c:choose>
			</shiro:hasPermission>
			<input id="btnCancel" class="btn" type="button" value="返 回" onclick = "window.location.href ='${ctx}/sys/role/back'"/>
		</div>
	</form:form>
</body>
</html>