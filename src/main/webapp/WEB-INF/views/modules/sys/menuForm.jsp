...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#inputForm").validate({
				rules:{
					id:{remote:"${ctx}/sys/menu/checkMenuId?oldId=" + encodeURIComponent('${menu.id}')}
				},
				messages: {
					id:{remote: "菜单编号已存在"}
				},
...
<body>
	...
	<form:form id="inputForm" modelAttribute="menu" action="${ctx}/sys/menu/save" method="post" class="form-horizontal">
		<form:hidden path="isNewRecord" value="${empty menu.id ?'true' :'false'}"/>
				...
				<c:choose>
					<c:when test="${not empty menu.id}">
						<form:input path="id" htmlEscape="false" maxlength="50"
							class="required digits" readonly="true"/>
					</c:when>
					<c:otherwise>
						<form:input path="id" htmlEscape="false" maxlength="50"
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="target" htmlEscape="false" maxlength="10" class=""/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="sort" htmlEscape="false" maxlength="24" class="required digits"/>
		...
		<div class="control-group">
			<label class="control-label">是否显示机构树:</label>
			<div class="controls">
				<form:radiobuttons path="treeNeed" items="${fns:getDictList('show_hide')}" itemLabel="label" itemValue="value" htmlEscape="false" class="required"/>
				<span class="help-inline">机构树是否显示到列表左侧</span>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">工作流分组:</label>
			<div class="controls">
			<form:select path="actCategory" itemLabel="label" itemValue="value" class="input-medium">
				<form:option value="">
					<spring:message code="common.select" />
				</form:option>
				<form:options id="actCategory" items="${fns:getDictList('act_category')}"
...
<%-- 		<div class="control-group">
			...
			<div class="controls">
				<form:input path="versionNo" htmlEscape="false" maxlength="4" class=""/>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div> --%>
		<div class="control-group">
		...
		<div class="form-actions">
			...
			<input id="btnCancel" class="btn" type="button" value="返 回" onclick = "window.location.href ='${ctx}/sys/menu/back'"/>
		</div>
	</form:form>
</body>
</html>