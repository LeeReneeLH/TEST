...
<head>
	<%-- 发行基金出库修改 --%>
	<title><spring:message code="allocation.issueFund.outStore.update" /></title>
	...
	<script type="text/javascript">
		$(document).ready(function() {
            ...
            $("#add").click(function () {
            	//清除tr背景色
            	clearTrBackgroundColor();
            	var url = "${ctx}/allocation/v02/pbocAllocatedQuota/add?userCacheId=" + $("#cacheId").val();
			...
			$("#btnApproval").click(function(){
                ...
                var url = "${ctx}/allocation/v02/pbocAllocatedQuota/aloneOption?targetStatus=22&strUpdateDate=" 
		...
		function updateItem(goodsId, goodsName, number) {	
			...
			//清除tr背景色
			clearTrBackgroundColor();
			var url = "${ctx}/allocation/v02/pbocAllocatedQuota/updateGoodsItem?userCacheId=" + $("#cacheId").val();
...
<body>
	<ul class="nav nav-tabs">
		...
		<%-- 发行基金出库修改 --%>
		<li class="active"><a href="#" onclick="javascript:return false;"><spring:message code="allocation.issueFund.outStore.update" /></a></li>
	</ul><br/>
					...
					<form:form id="tempform" class="form-horizontal">
						...
						<div class="control-group item">
							...
							<div class="controls" style="margin-left: 90px;">
							   	...
							   	value="<fmt:formatDate value="${allocatedQuotaSession.applyDate}" pattern="yyyy-MM-dd"/>"  />
							</div>
						</div>
						<div class="clear"></div>
						<div class="control-group item">
		...
		<div class="span9" >
			<div class="row" >
				<div class="span9">
					<div class="alert alert-info">
						<%-- 配款明细 --%>
						<b><spring:message code="allocation.quota.detail" /></b>
					</div>
				</div>
			</div>
			<div class="row">
			...
			<div class="row">
				<div class="span8" style="text-align: left">
					<%-- 配款总金额 --%>
					<label ><spring:message code='allocation.cash.quota.amount.all'/>：</label>
					 ...
					 <div style="overflow-y: auto; height: 315px;">
						...
						<table id="contentTable" class="table table-hover">
							<thead>
								<tr>
									<%-- 配款明细 --%>
									<th style="text-align: center" colspan="5"><spring:message code="allocation.quota.detail" /></th>
								</tr>
								<tr>
									<%-- 序号 --%>
									<th style="text-align: center" ><spring:message code="common.seqNo" /></th>
									...
									<c:if test="${item.registType == '10'}">
										...
										<tr id="${item.goodsId}">
											...
											<td style="text-align:center;">
												<a href="${ctx}/allocation/v02/pbocAllocatedQuota/deleteGoods?goodsId=${item.goodsId}&userCacheId=${userCacheId }"
			...
			<div class="row">
				<div class="span9">
					<div class="form-actions" style="text-align: right;">
						<%-- 提交 --%>
						<input type="button" id="btnApproval" value="<spring:message code='common.commit' />" class="btn btn-primary">&nbsp;
						<%-- 返回 --%>
						<input id="btnCancel" class="btn" type="button"
							value="<spring:message code='common.return'/>"
							onclick="window.location.href='${ctx}/allocation/v02/pbocAllocatedQuota/back'"/>
					</div>
				</div>
