...
<head>
	<%-- 申请下拨列表 --%>
	<title><spring:message code="allocation.application.allocated.list" /></title>
...
<body>
	<ul class="nav nav-tabs">
		<%-- 申请下拨列表(link) --%>
		<li class="active"><a href="#" onclick="javascript:return false;"><spring:message code="allocation.application.allocated.list" /></a></li>
		<%-- 申请下拨登记(link) --%>
	...
	<table  id="contentTable" class="table table-hover">
		...
		<tbody>
			<c:forEach items="${page.list}" var="allocation" varStatus="status">
				<tr>
					<%-- 流水单号 --%>
					<td><a href="${ctx}/allocation/v02/pbocWorkflow/findApplicationAllocationStatus?allId=${allocation.allId}">${allocation.allId}</a></td>
					<%-- 申请金额 --%>
					...
					<td>
						<c:if test="${allocation.status == '20'}">
							<a href="${ctx}/allocation/v02/pbocAllocatedOrder/form?allId=${allocation.allId}&pageType=bussnessApplicationEdit" title="编辑">
								<%-- <spring:message code="common.modify" /> --%><i class="fa fa-edit  text-green fa-lg"></i>
							</a>
							&nbsp;
							<a href="${ctx}/allocation/v02/pbocAllocatedOrder/delete?allId=${allocation.allId}&pageType=bussnessApplicationList&strUpdateDate=${allocation.strUpdateDate}"
						...
						<c:if test="${allocation.status == '41' or allocation.status == '42'}">
							<a href="${ctx}/allocation/v02/pbocAllocatedOrder/confirmAccept?allId=${allocation.allId}&targetStatus=99&pageType=bussnessApplicationList"
								onclick="return confirmx('<spring:message code="message.I2013" />', this.href)" title="接收确认">
								<%-- 接收确认 --%>
								<%-- <spring:message code="common.receiveConfirm" /> --%><i class="fa fa-check-square-o   text-green fa-lg"></i>
							</a>
							&nbsp;
						</c:if>
						<c:choose>
							<c:when test="${allocation.status == '20'}">
								<a href="${ctx}/allocation/v02/pbocAllocatedOrder/form?allId=${allocation.allId}&pageType=bussnessApplicationEdit" title="编辑"><i class="fa fa-eye fa-lg"></i></a>
							</c:when>
							<c:otherwise>
								<a href="${ctx}/allocation/v02/pbocAllocatedOrder/form?allId=${allocation.allId}&pageType=bussnessApplicationView" title="查看"><i class="fa fa-eye fa-lg"></i></a>
							</c:otherwise>
						</c:choose>
					</td>
				</tr>
