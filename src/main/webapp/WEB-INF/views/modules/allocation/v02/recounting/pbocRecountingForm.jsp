...
<head>
	<%-- 发行基金复点出库登记 --%>
	<title><spring:message code="allocation.pboc.recounting.outStore.register" /></title>
	...
	<script type="text/javascript">
		$(document).ready(function() {
            ...
            $("#add").click(function () {
            	//清除tr背景色
            	clearTrBackgroundColor();
            	var url = "${ctx}/allocation/v02/pbocRecounting/add?userCacheId=" + $("#cacheId").val();
			...
			$("#btnApproval").click(function(){
                ...
                var url = "${ctx}/allocation/v02/pbocRecounting/save?userCacheId=" + $("#cacheId").val();
	              $("#inputForm").attr("action", url);
				  $("#inputForm").submit();
			});
		...
		function updateItem(goodsId, goodsName, number) {	
			...
			//清除tr背景色
			clearTrBackgroundColor();
			var url = "${ctx}/allocation/v02/pbocRecounting/updateGoodsItem?userCacheId=" + $("#cacheId").val();
		...
		function inputAmountChange() {
			var goodsValue = $("#goodsVal").val();
			
			var inputAmount = $("#inputAmount").val();
			
			var goodsNumber = Math.ceil(inputAmount / goodsValue);
			//var goodsNumber = inputAmount / goodsValue;
			$("#moneyNumber").val(goodsNumber);
		}
	</script>
	<style type="text/css">
		.table thead tr { 
...
<body>
	<ul class="nav nav-tabs">
		...
		<%-- 发行基金复点出库登记 --%>
		<li class="active"><a href="#" onclick="javascript:return false;"><spring:message code="allocation.pboc.recounting.outStore.register" /></a></li>
	</ul><br/>
		...
		<div class="row">
			...
			<div class="span12">
				<div class="control-group item">
					...
					<label>
						<form:select path="businessType" id="businessType" class="input-small" style="width:150px;">
							<form:options items="${fns:getFilterDictList('all_businessType', true, '56')}"
			...
			<div class="span12">
				...
				<div class="control-group item">
					<%-- 清分机构 --%>
					<label class="control-label" style="width:100px;"><spring:message code="common.clearOffice" />：</label>
					<label>
				   		<sys:treeselect id="aofficeId" name="aoffice.id"
							value="${pbocAllAllocateInfo.aoffice.id}" labelName="aoffice.name"
							labelValue="${pbocAllAllocateInfo.aoffice.name}" title="<spring:message code='allocation.application.office' />"
							url="/sys/office/treeData" cssClass="required input-small" 
		...
		<div class="row">
			...
			<div class="span10">
				...
				<sys:pbocgoodselect currencyReserve="${fns:getConfig('sto.relevance.currency.cny')}"
			     classificationReserve="${fns:getConfig('sto.goods.classification.damaged.recounting.no')}"/>
			    <div class="clear"></div>
			    <input id="goodsVal" type="hidden">
			    <%-- 物品金额 --%>
			    <div class="control-group item">
					<label class="control-label" style="width: 80px;"><spring:message
							code="common.amount" />：</label>
					<div class="controls" style="margin-left: 90px;">
						<input id="inputAmount" maxlength="9" style="width:117px;text-align:right;border-radius:4px;border:1px solid #ccc;height: 20px;padding: 4px 6px;line-height: 20px;" class="digits" 
						onkeyup="inputAmountChange();"/>
					</div>
				</div>
				<%-- 物品数量 --%>
				<div class="control-group item">
					...
					<div class="controls" style="margin-left: 90px;">
						<form:input id="moneyNumber"
							...
							maxlength="3" style="width:117px;text-align:right;" class="digits" />
						...
						<c:forEach items="${fns:getCache(userCacheId, null).pbocAllAllocateItemList}" var="item" varStatus="status">
							<tr id="${item.goodsId}">
								...
								<td style="text-align:center;">
									<a href="${ctx}/allocation/v02/pbocRecounting/deleteGoods?goodsId=${item.goodsId}&userCacheId=${userCacheId }"
		...
		<div class="span12">
			<div class="form-actions" style="width:740px;">
				...
				<input id="btnCancel" class="btn" type="button"
					...
					onclick="window.location.href='${ctx}/allocation/v02/pbocRecounting/back'"/>
			</div>
		</div>
