...
<body>
	<div style="margin-top: 5px; margin-left: 5px; margin-right: 5px;">
		...
		<div class="clear"></div>
		<!-- 交接信息 -->
		<c:if test="${operateInfo.status eq '42' or operateInfo.status eq '99'}">
			<!-- 交接授权人 -->
		...
		<c:if test="${operateInfo.status == '20'}">
		...
		<c:if test="${operateInfo.businessType eq 51 and operateInfo.status eq '40'}">
			<div class="bg-light-blue disabled color-palette">
				<span style="margin-left: 20px"><spring:message
						code="store.goodsInfo" /></span>
			</div>
		...
		<c:if test="${((operateInfo.businessType eq 50 or operateInfo.businessType eq 52) and operateInfo.status eq '40') or operateInfo.status eq '22'}">
			<div class="bg-light-blue disabled color-palette">
				<span style="margin-left: 20px"><spring:message
						code="store.goodsInfo" /></span>
			</div>
				...
				<c:forEach items="${historyList}" var="items">
					<tbody>
						<tr>
							...
							<c:if test="${operateInfo.businessType eq 51}">
								<td style="text-align: center;">${fns:getDictLabel(items.status,'pboc_order_quota_status',"")}</td>
							</c:if>
							<c:if test="${operateInfo.businessType eq 50 or operateInfo.businessType eq 52}">
								<td style="text-align: center;">${fns:getDictLabel(items.status,'pboc_order_handin_status',"")}</td>
							</c:if>
