...
<head>
	<%-- 申请上缴登记 --%>
	<title><spring:message code="allocation.application.handin.registe" /></title>
	...
	<script type="text/javascript">
		$(document).ready(function() {
            ...
            $("#add").click(function () {
            	//清除tr背景色
            	clearTrBackgroundColor();
            	var url = "${ctx}/allocation/v02/pbocHandinOrder/add?userCacheId=" + $("#cacheId").val();
			...
			$("#btnSubmit").click(function(){
				...
				$("#inputForm").attr("action","${ctx}/allocation/v02/pbocHandinOrder/save?pageType=bussnessApplicationList&strUpdateDate=" 
		...
		function updateItem(goodsId, goodsName, number) {
			//清除tr背景色
			clearTrBackgroundColor();
			var url = "${ctx}/allocation/v02/pbocHandinOrder/updateGoodsItem?userCacheId=" + $("#cacheId").val();
...
<body>
	<ul class="nav nav-tabs">
		...
		<%-- 申请上缴登记 --%>
		<li class="active"><a href="#" onclick="javascript:return false;"><spring:message code="allocation.application.handin.registe" /></a></li>
	</ul><br/>
						...
						<c:forEach items="${fns:getCache(userCacheId, null).pbocAllAllocateItemList}" var="item" varStatus="status">
							<tr id="${item.goodsId}">
								...
								<td style="text-align:right;">
									<a href="#" onclick="updateItem('${item.goodsId}','${item.goodsName}', '${item.moneyNumber}');javascript:return false;">${item.goodsName}</a>
								</td>
								...
								<td style="text-align:center;">
									<a href="${ctx}/allocation/v02/pbocHandinOrder/deleteGoods?goodsId=${item.goodsId}&userCacheId=${userCacheId }"
		...
		<div class="span9">
			<div class="form-actions">
				...
				<input id="btnCancel" class="btn" type="button"
					...
					onclick="window.location.href='${ctx}/allocation/v02/pbocHandinOrder/back?pageType=bussnessApplicationList'"/>
			</div>
		</div>
