	...
	<script type="text/javascript">
		$(document).ready(function() {
	        ...
	        $("#add").click(function () {
	        	//清除tr背景色
	        	clearTrBackgroundColor();
	        	var url = "${ctx}/allocation/v01/cashBetweenDtoDC/addClear?userCacheId=" + $("#cacheId").val();
			...
			$("#btnSubmit").click(function(){
				...
				$("#inputForm").attr("action","${ctx}/allocation/v01/cashBetweenDtoDC/saveClearRegisterInfo?strUpdateDate=" 
		...
		function updateItems(goodsId, goodsName, number) {	
			//清除tr背景色
			clearTrBackgroundColor();
			var url = "${ctx}/allocation/v01/cashBetweenDtoDC/updateClearGoodsItem?userCacheId=" + $("#cacheId").val();
...
<body>
	...
	<form:form id="inputForm" modelAttribute="allAllocateInfo" action="" method="post" class="form-horizontal">
		...
		<%-- 自定义控件 --%>
		<sys:goodselect type="good_manager" classificationReserve="${fns:getConfig('sto.goods.classification.circulation')},${fns:getConfig('sto.goods.classification.damaged')},${fns:getConfig('sto.goods.classification.counterfeit')},${fns:getConfig('sto.goods.classification.waitClear')}"/>
		<%-- 数量 --%>
		<div class="control-group item">
			...
			<tbody>
				<c:forEach items="${fns:getCache(userCacheId, null).allAllocateItemList}" var="item" varStatus="status">
					<tr>
						...
						<td style="text-align:center">
							<a href="${ctx}/allocation/v01/cashBetweenDtoDC/deleteClearGoods?goodsId=${item.goodsId}&userCacheId=${userCacheId }"
