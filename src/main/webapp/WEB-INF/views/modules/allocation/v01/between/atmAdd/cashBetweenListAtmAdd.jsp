...
<head>
	<%-- 库间配钞列表 --%>
	<title><spring:message code="allocation.cashBetween.atm.add.list" /></title>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$("#btnSubmit").click(function(){
				$("#searchForm").attr("action", "${ctx}/allocation/v01/cashBetweenAtmAdd/list");
				$("#searchForm").submit();
			});
		});
		...
		function printApprovalGoodDetail(allId){
			var content = "iframe:${ctx}/allocation/v01/cashBetweenAtmAdd/printRegisterGoodsDetail?allId=" + allId;
			top.$.jBox.open(
				content,
				//打印
...
<body>
	<ul class="nav nav-tabs">
		<%-- 库间配钞列表 --%>
		<li class="active"><a href="#" onclick="javascript:return false;"><spring:message code="allocation.cashBetween.atm.add.list" /></a></li>
		<shiro:hasPermission name="allocation:cashStoreAddAtm:edit">
			<%-- 出库登记 --%>
			<li><a href="${ctx}/allocation/v01/cashBetweenAtmAdd/toCashOutStoreForm?businessType=${allAllocateInfo.businessType}"><spring:message code="allocation.outStore.register" /></a></li>
		</shiro:hasPermission>
		...
		<form:form id="searchForm" modelAttribute="allAllocateInfo" action="" method="post" class="breadcrumb form-search">
			...
			<div class="row">
				<div class="span14" style="margin-top:5px">
					...
					<%-- 接收机构 --%>
					<label><spring:message code="allocation.instore.office" />：</label>
					<sys:treeselect id="aOfficeId" name="aOffice.id"
							value="${allAllocateInfo.aOffice.id}" labelName="aOffice.name"
							labelValue="${allAllocateInfo.aOffice.name}" title="<spring:message code='allocation.instore.office' />"
							url="/sys/office/treeData" cssClass="required input-small"
					...
					<form:select path="status" id="status" class="input-medium" >
						...
						<form:options items="${fns:getFilterDictList('transfer_status', false, '12,13')}"
				...
				<div class="span7" style="margin-top:5px">
					...
					&nbsp;
					<%-- 查询 --%>
					<input id="btnSubmit" class="btn btn-primary" type="submit" style="margin-left:20px;" value="<spring:message code='common.search'/>"/>
				</div>
			</div>
		</form:form>
	</div>
					...
					<td>
						<shiro:hasPermission name="allocation:cashStoreAddAtm:edit">
							<c:if test="${allocate.status == '11' }">
								...
								<a href="${ctx}/allocation/v01/cashBetweenAtmAdd/toCashOutStoreForm?allId=${allocate.allId}" title="<spring:message code='common.modify' />" title="<spring:message code="common.edit" />">
									<i class="fa fa-edit text-green fa-lg"></i>
								</a>
								<a href="${ctx}/allocation/v01/cashBetweenAtmAdd/delete?allId=${allocate.allId}"
									onclick="return confirmx('<spring:message code="message.I0001"/>', this.href)" title="<spring:message code='common.delete' />" title="<spring:message code='common.delete' />">
									<i class="fa fa-trash-o text-red fa-lg"></i>
								</a>
							</c:if>
						</shiro:hasPermission>
						<shiro:hasPermission name="allocation:cashClassficationRoomAddAtm:edit">
							<c:if test="${allocate.status == '11' }">
								<a href="${ctx}/allocation/v01/cashBetweenAtmAdd/toClearCenterReceiveForm?allId=${allocate.allId}" title="<spring:message code='common.receiveConfirm' />">
									<i class="fa fa-check-square-o fa-lg"></i>
								</a>
							</c:if>
						</shiro:hasPermission>
					
						<c:if test="${allocate.status == '13' || allocate.status == '99'}">
							<a href="${ctx}/allocation/v01/cashBetweenAtmAdd/cashShowDetail?allId=${allocate.allId}" title='<spring:message code='common.view' />'>
