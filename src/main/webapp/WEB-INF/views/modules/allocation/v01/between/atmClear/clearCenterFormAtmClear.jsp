<%@ page contentType="text/html;charset=UTF-8" %>
<%@ include file="/WEB-INF/views/include/taglib.jsp"%>
<html>
<head>
	<%-- 入库登记 --%>
	<title><spring:message code="allocation.inStore.register" /> </title>
	<meta name="decorator" content="default"/>
	<script type="text/javascript" src="${ctxStatic}/script/modules/allocation/v01/common/updateGoodsItem.js"></script>
	<script type="text/javascript">
		$(document).ready(function() {
			// 清除tr背景色
			clearTrBackgroundColor();
			$("#inputForm").validate(
				{   submitHandler : function(form) {
						loading('正在提交，请稍等...');
						form.submit();
					},
					//errorContainer : "#messageBox",
					errorPlacement : function(error, element) {
						//$("#messageBox").text("输入有误，请先更正。");
						if (element.is(":checkbox")
								|| element.is(":radio")
								|| element.parent().is(
										".input-append")) {
							error.appendTo(element.parent()
							.parent());
						} else {
							error.insertAfter(element);
						}
					}
			   });
			 
			// 添加物品
	        $("#add").click(function () {
	        	//清除tr背景色
	        	clearTrBackgroundColor();
	        	var url = "${ctx}/allocation/v01/cashBetweenAtmClear/addClear?userCacheId=" + $("#cacheId").val();
	        	$("#inputForm").attr("action", url);
				$("#inputForm").submit();
	        });
			
	     	// 提交按钮
			$("#btnSubmit").click(function(){
				$("#moneyNumber").val("");
				$("#moneyNumber").removeClass();
				$("#inputForm").attr("action","${ctx}/allocation/v01/cashBetweenAtmClear/saveClearRegisterInfo?strUpdateDate=" 
						+ $("#strUpdateDate").val() + "&userCacheId=" + $("#cacheId").val());
				$("#inputForm").submit();
			});
	     	
		});
		
		function updateItems(goodsId, goodsName, number, boxNo) {	/** 添加箱袋编号参数 boxNo  by:wxz 2018-01-16*/
			//清除tr背景色
			clearTrBackgroundColor();
			var url = "${ctx}/allocation/v01/cashBetweenAtmClear/updateClearGoodsItem?userCacheId=" + $("#cacheId").val()+ "&boxNo=" + boxNo;
			updateGoodsItem(goodsId, goodsName, number, url);
		}	
	
		//清除tr背景色
		function clearTrBackgroundColor(){
			$("tr").removeClass();
		}
	</script>
</head>
<body>
	<ul class="nav nav-tabs">
		<%-- 库间清机入库列表 --%>
		<li><a href="${ctx}/allocation/v01/cashBetweenAtmClear/list"><spring:message code="allocation.cashBetween.atm.clear.inStore.list" /></a></li>
		<%-- 入库登记 --%>
		<li class="active"><a href="#" onclick="javascript:return false;"><spring:message code="allocation.inStore.register" /></a></li>
	</ul>
	<form:form id="inputForm" modelAttribute="allAllocateInfo" action="" method="post" class="form-horizontal">
		<sys:message content="${message}"/>
		<form:hidden path="allId"/>
		<form:hidden path="businessType"/>
		<c:if test="${fns:getUser().office.type != '6'}">
			<div class="control-group item">
				<%-- 登记机构 --%>
				<label class="control-label" style="width:80px;"><spring:message code="allocation.register.office" />：</label>
				<div class="controls" style="margin-left: 90px;">
					<sys:treeselect id="rOfficeId" name="rOffice.id"
						value="${allAllocateInfo.rOffice.id}" labelName="rOffice.name"
						labelValue="${allAllocateInfo.rOffice.name}" title="<spring:message code='allocation.register.office' />"
						url="/sys/office/treeData" cssClass="required input-small"
						notAllowSelectParent="false" notAllowSelectRoot="false" type="6"
					    isAll="true" />
				</div>
			</div>
		</c:if>
		<div class="control-group item">
			<%-- 接收机构 --%>
			<label class="control-label" style="width:100px;"><spring:message code="allocation.accept.office" />：</label>
			<div class="controls" style="margin-left: 110px;">
				<sys:treeselect id="aOfficeId" name="aOffice.id"
					value="${allAllocateInfo.aOffice.id}" labelName="aOffice.name"
					labelValue="${allAllocateInfo.aOffice.name}" title="<spring:message code='allocation.accept.office' />"
					url="/sys/office/treeData" cssClass="required input-small"
					notAllowSelectParent="false" notAllowSelectRoot="false" type="3"
				    isAll="true" />
			</div>
		</div>
		<div class="clear"></div>
		<HR style="FILTER: alpha(opacity=100,finishopacity=0,style=1)" align="left" width="100%" color="#987cb9" SIZE="1">
		<%-- 自定义控件 --%>
		<%--单位保留'张' qph 2017-12-14 begin --%>
		<sys:goodselect type="good_manager" classificationReserve="${fns:getConfig('sto.goods.classification.circulation')},${fns:getConfig('sto.goods.classification.atm')},${fns:getConfig('sto.goods.classification.damaged')},${fns:getConfig('sto.goods.classification.counterfeit')},${fns:getConfig('sto.goods.classification.waitClear')}"
		cashReserve="${fns:getConfig('sto.goods.cash.paper')}" denominationReserve="${fns:getConfig('sto.goods.denomination.cnyp100')},${fns:getConfig('sto.goods.denomination.cnyp50')}"
		unitReserve="${fns:getConfig('sto.goods.unit.sheet')}"/>
		<%--end --%>
		<%-- 数量 --%>
		<div class="control-group item">
			<label class="control-label" style="width:80px;"><spring:message code="common.number" />：</label>
			<label>
				<form:input id="moneyNumber" path="allAllocateItem.moneyNumber" htmlEscape="false" onkeyup="value=value.replace(/\D|^0/g,'')" style="width:150px;" maxlength="5" class="digits required" />
				<span class="help-inline"><font color="red">*</font> </span>
			</label>
	    </div>
	    <div class="clear"></div>
	    <%-- 箱号 --%>
		<div class="control-group item">
			<label class="control-label" style="width:80px;"><spring:message code="allocation.boxNo" />：</label>
			<label>
				<form:input path="allAllocateItem.boxNo" htmlEscape="false" style="width:150px;" maxlength="16"/>
			</label>
	    </div>
	    
	    <div class="clear"></div>
	    <HR style="FILTER: alpha(opacity=100,finishopacity=0,style=1)" align="left" width="100%" color="#987cb9" SIZE="1">
	</form:form>
	<input type="hidden" id="strUpdateDate" name="strUpdateDate" value="${allAllocateInfo.strUpdateDate}">
	<div class ="row" style="margin-left:5px">
		<%-- 添加 --%>
		<div class="span1" style="text-align: left">
			<input id="add" class="btn btn-primary" type="button" value="<spring:message code='common.add'/>" />
		</div>
	    <div class="span8" style="text-align: right">
			<%-- 登记总金额 --%>
			<label style="padding-bottom:10px;"><spring:message code="allocation.atm.register.amount" />：</label>
			<input type="text" id="registerAmountShow" value="<fmt:formatNumber value="${allAllocateInfo.registerAmount}" 
			pattern="#,##0.00#" />" class="input-medium" style="text-align:right;width:120px;" readOnly="readOnly"/>
			<%-- 清分总金额(格式化) --%>
			<label style="margin-left:10px;padding-top:10px;">${sto:getUpperAmount(allAllocateInfo.registerAmount)}</label>
    	</div>
    	
	</div>
	<%-- 入库现金详细信息 --%>
	<div style="margin-left:25px;height:205px;width:1000px;overflow:auto;overflow-x:hidden;">
		<table class="table  table-hover">
			<thead>
				<tr>
					<%-- 序号 --%>
					<th style="text-align:center"><spring:message code="common.seqNo" /></th>
					<%-- 物品名称 --%>
					<th style="text-align:center"><spring:message code="store.goodsName" /></th>
					<%-- 数量 --%>
					<th style="text-align:center"><spring:message code="common.number" /></th>
					<%-- 金额 --%>
					<th style="text-align:center"><spring:message code="common.amount" /></th>
					<%-- RFID --%>
					<th style="text-align:center;"><spring:message code="allocation.boxNo" /></th>
					<%-- display:${empty item.rfid ? none:block} --%>
					<%-- 批次号 --%>
					<th style="text-align:center"><spring:message code="allocation.batch.number" /></th>
					<%-- 操作 --%>
					<th style="text-align:center"><spring:message code='common.operation'/></th>
				</tr>
			</thead> 
			<tbody>
				<c:forEach items="${fns:getCache(userCacheId, null).allAllocateItemList}" var="item" varStatus="status">
					<tr>
						<%-- 序号 --%>
						<td style="text-align:center">
							${status.index + 1}
						</td>
						<%-- 物品名称 --%>
						<td style="text-align:center">
							<a href="#" onclick="updateItems('${item.goodsId}','${sto:getGoodsName(item.goodsId)}','${item.moneyNumber}','${item.boxNo}');"
							>${sto:getGoodsName(item.goodsId)}</a>
						</td>
						<%-- 数量 --%>
						<td  style="text-align:right;">
							${item.moneyNumber}
						</td>
						<%-- 金额 --%>
						<td style="text-align:right">
							<fmt:formatNumber value="${item.moneyAmount}" pattern="#,##0.00#"/>
						</td>
						<%-- 箱号--%>
						<td  style="text-align:right;">
							${item.boxNo}
						</td>
						<%-- 批次号--%>
						<td  style="text-align:right;">
							${item.batchNo}
						</td>
						<td style="text-align:center">
							<!-- 添加箱号字段(boxNo)，传到后台		by:wxz	2018-01-12 -->
							<a href="${ctx}/allocation/v01/cashBetweenAtmClear/deleteClearGoods?goodsId=${item.goodsId}&userCacheId=${userCacheId }&boxNo=${item.boxNo}"
								onclick="return confirmx('<spring:message code="message.I0001"/>', this.href)" title="<spring:message code='common.delete' />">
								<i class="fa fa-trash-o text-red fa-lg"></i>
							</a>
						</td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
	</div>
	<input type="hidden" id="cacheId" value="${userCacheId }">
	<div class="form-actions" style="padding-left: 25px">
		<%-- 保存 --%>
		<input id="btnSubmit" class="btn btn-primary" type="button"  value="<spring:message code='common.commit'/>" />
		&nbsp;
		<%-- 返回 --%>
		<input id="btnCancel" class="btn" type="button"
			value="<spring:message code='common.return'/>"
			onclick="window.location.href='${ctx}/allocation/v01/cashBetweenAtmClear/back'"/>
	</div>
</body>
</html>
