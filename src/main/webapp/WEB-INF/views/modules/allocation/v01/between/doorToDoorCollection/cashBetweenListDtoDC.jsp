...
<head>
<%-- 上门收款入库列表 --%>
<title><spring:message code="allocation.cashBetween.doorToDoorReceive.inStore.list" /></title>
<meta name="decorator" content="default" />
<script type="text/javascript" src="${ctxStatic}/common/common.js"></script>
<script type="text/javascript">
	$(document).ready(function() {
		$("#btnSubmit").click(function() {
			$("#searchForm").attr("action",	"${ctx}/allocation/v01/cashBetweenDtoDC/list");
			$("#searchForm").submit();
		});
	});
</script>
...
<body>
	<ul class="nav nav-tabs">
		<%-- 上门收款入库列表 --%>
		<li class="active"><a href="#" onclick="javascript:return false;"><spring:message code="allocation.cashBetween.doorToDoorReceive.inStore.list" /></a></li>
		<shiro:hasPermission name="allocation:cashClassficationRoomDtoD:edit">
			<%-- 入库登记 --%>
			<li><a
		...
		<tbody>
			<c:forEach items="${page.list}" var="allocate"
				...
				<tr>
					...
					<%-- 操作 --%>
					<td>
						<shiro:hasPermission name="allocation:cashStoreDtoD:edit">
							<c:if test="${allocate.status == '13' }">
								<%-- 入库确认 --%>
								<a href="${ctx}/allocation/v01/cashBetweenDtoDC/toCashInStoreReceive?allId=${allocate.allId}" title="<spring:message code='allocation.inStore.confirm' />">
						...
						<shiro:hasPermission	name="allocation:cashClassficationRoomDtoD:edit">
							<c:if test="${allocate.status == '13' }">
								<%-- 修改 --%>
								<a href="${ctx}/allocation/v01/cashBetweenDtoDC/toClearCenterForm?allId=${allocate.allId}" title='<spring:message code='common.modify' />'>
									<i class="fa fa-edit text-green fa-lg"></i>
								</a>
								<%-- 删除 --%>
								<a href="${ctx}/allocation/v01/cashBetweenDtoDC/delete?allId=${allocate.allId}"
						 ...
						 <c:if
							...
							<%-- 查看 --%>
							<a href="${ctx}/allocation/v01/cashBetweenDtoDC/cashShowDetail?allId=${allocate.allId}" title='<spring:message code='common.view' />'>
