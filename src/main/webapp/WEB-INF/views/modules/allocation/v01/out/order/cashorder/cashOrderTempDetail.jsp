...
<head>
<title><c:choose>
		<c:when test="${allAllocateInfo.pageType == 'pointTempEdit' or allAllocateInfo.pageType == 'pointTempAdd'}">
			<c:if test="${allAllocateInfo.pageType == 'pointTempAdd'}">
				<%-- 临时预约登记 --%>
				<spring:message code="allocation.cash.order.temporary" />
			</c:if>
			<c:if test="${allAllocateInfo.pageType == 'pointTempEdit'}">
				<%-- 现金预约修改 --%>
				<spring:message code="allocation.cash.order.temporary.update" />
			</c:if>
		</c:when>
	</c:choose></title>
					...
					function() {
						...
						$("#btnSubmit").click(
										function() {
												...
												$("#moneyNumberOrder").val("");
												$("#moneyNumberOrder").removeClass();
												$("#inputForm")
														.attr("action",
																"${ctx}/allocation/v01/cashOrder/save?saveType=save");
												$("#inputForm").submit();
...
<li><a href="${ctx}/allocation/v01/cashOrder/form?pageType=pointAdd"><spring:message code="allocation.cash.order.register"/></a></li>
					...
					<c:choose>
						...
						<c:otherwise>
							<c:if test="${allAllocateInfo.pageType == 'pointTempAdd'}">
								<li class="active"><a href="#"
									onclick="javascript:return false;"><spring:message
									code="allocation.cash.order.temporary" /></a></li>
							</c:if>
							<c:if test="${allAllocateInfo.pageType == 'pointTempEdit'}">
								<li class="active"><a href="#"
									onclick="javascript:return false;"><spring:message
									code="allocation.cash.order.temporary.update" /></a></li>
							</c:if>
						</c:otherwise>
					</c:choose>
				</c:when>
	...
	<form:form id="inputForm" modelAttribute="allAllocateInfo" action=""
		...
		<c:choose>
			<%-- 编辑状态 显示 --%>
			<c:when
				test="${allAllocateInfo.pageType eq 'pointTempEdit' || allAllocateInfo.pageType eq 'pointTempAdd' }">
				<%-- 网点端显示 --%>
				<shiro:hasPermission name="allocation:point:edit">
					<c:choose>
						<c:when test="${allAllocateInfo.pageType == 'pointTempEdit' or allAllocateInfo.pageType == 'pointTempAdd'}">
	...
	<%-- 现金详细信息 --%>
		...
		<div id="detailDiv"
			style="margin-left: 0px; margin-top:15px; height: 305px; width: 800px; overflow: auto; overflow-x: hidden;">
			<shiro:hasPermission name="allocation:point:edit">
				<c:choose>
					<c:when test="${ allAllocateInfo.pageType == 'pointTempEdit' or allAllocateInfo.pageType == 'pointTempAdd'}">
						<%-- 预约总金额 --%>
								...
								<c:when
									...
									<td style="text-align: center">
										...
										<shiro:hasPermission
											...
											<c:choose>
												<c:when test="${allAllocateInfo.pageType == 'pointTempEdit' or allAllocateInfo.pageType == 'pointTempAdd'}">
													<a href="${ctx}/allocation/v01/cashOrder/save?saveType=delete&detailId=${allAllocateItem.value.allItemsId}&allId=${allocationOrderCash.allId}&pagetype=${allAllocateInfo.pageType}"
