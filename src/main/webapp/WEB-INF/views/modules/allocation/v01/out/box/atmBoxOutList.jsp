	...
	<script type="text/javascript">
		$(document).ready(function() {
			$("#btnSubmit").click(function(){
				$("#searchForm").attr("action", "${ctx}/allocation/v01/atmBoxHandOut/list");
				$("#searchForm").submit();
			});
		});
		...
		function showDetail(allId) {
			var content = "iframe:${ctx}/allocation/v01/atmBoxHandOut/getAllAllocateDetail?allId="+allId;
			top.$.jBox.open(
					content,
					"查看详情", 800, 700, {
						buttons : {
...
<body>
	<ul class="nav nav-tabs">
		<!-- 钞箱出库列表 -->
		<li class="active"><a href="${ctx}/allocation/v01/atmBoxHandOut/list">钞箱出库列表</a></li>
	</ul>
	<div class="row">
	...
	<table id="contentTable" class="table table-hover">
		...
		<tbody>
		<c:forEach items="${page.list}" var="allAllocateInfo">
			<tr>
				<!-- 流水单号 -->
				<td>${allAllocateInfo.allId}</td>
				<!-- 业务类型 -->
				<td>${fns:getDictLabel(allAllocateInfo.businessType,'all_businessType',"")}</td>
				<!-- 交接数量 -->
				...
				<!-- 操作 -->
				<shiro:hasPermission name="allocation:atmBoxHandOut:view"><td>
    				<!-- 查看 -->
    				<a href="" title="<spring:message code='common.view'/>" onclick="showDetail('${allAllocateInfo.allId}');javascript:return false;">
