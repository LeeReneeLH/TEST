...
<head>
	<%-- 出库登记 --%>
	<title><spring:message code="allocation.outStore.register" /> </title>
	...
	<script type="text/javascript">
		$(document).ready(function() {
	        ...
	        $("#add").click(function () {
	        	//清除tr背景色
	        	clearTrBackgroundColor();
	        	var url = "${ctx}/allocation/v01/cashBetweenAtmAdd/addRegister?userCacheId=" + $("#cacheId").val();
			...
			$("#btnSubmit").click(function(){
				...
				$("#inputForm").attr("action","${ctx}/allocation/v01/cashBetweenAtmAdd/saveRegisterInfo?strUpdateDate=" 
		...
		function updateItems(goodsId, goodsName, number) {	
			//清除tr背景色
			clearTrBackgroundColor();
			var url = "${ctx}/allocation/v01/cashBetweenAtmAdd/updateRegisterGoodsItem?userCacheId=" + $("#cacheId").val();
...
<body>
	<ul class="nav nav-tabs">
		...
		<%-- 出库登记 --%>
		<li class="active"><a href="#" onclick="javascript:return false;"><spring:message code="allocation.outStore.register" /></a></li>
	</ul>
	<form:form id="inputForm" modelAttribute="allAllocateInfo" action="" method="post" class="form-horizontal">
		...
		<div class="control-group item">
			<%-- 配钞机构 --%>
			<label class="control-label" style="width:100px;"><spring:message code="allocation.atm.add.office" />：</label>
			<div class="controls" style="margin-left: 110px;">
				<sys:treeselect id="aOfficeId" name="aOffice.id"
					value="${allAllocateInfo.aOffice.id}" labelName="aOffice.name"
					labelValue="${allAllocateInfo.aOffice.name}" title="<spring:message code='clear.orderClear.office' />"
					url="/sys/office/treeData" cssClass="required input-small"
		...
		<%-- 自定义控件 --%>
		<div id="waitClear">
			<sys:goodselect type="good_manager" currencyReserve="${fns:getConfig('sto.relevance.currency.cny')}" 
			classificationReserve="${fns:getConfig('sto.goods.classification.atm')}" 
			cashReserve="${fns:getConfig('sto.goods.cash.paper')}"/>
		</div>
		<%-- 数量 --%>
		<div class="control-group item">
	...
	<div class ="row" style="margin-left:10px">
	    ...
	    <div class="span8" style="text-align: right">
			<%-- 登记总金额 --%>
			<label style="padding-bottom:5px;"><spring:message code="allocation.atm.register.amount" />：</label>
			...
			<tbody>
				<c:forEach items="${fns:getCache(userCacheId, null).allAllocateItemList}" var="item" varStatus="status">
					<tr>
						...
						<td style="text-align:center">
							<a href="${ctx}/allocation/v01/cashBetweenAtmAdd/deleteRegisterGoods?goodsId=${item.goodsId}&userCacheId=${userCacheId }"
