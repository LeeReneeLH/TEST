...
var ProcessDiagramGenerator = {	
	...
	init: function(){
      ...
      if (label)
		...
		this.activityDrawInstructions["userTask"] = function(){
			...
			var processDiagramCanvas = this.processDiagramCanvas;
			processDiagramCanvas.setConextObject(activityImpl);
			var id = activityImpl.id;
			if (id.indexOf("ext") != -1 || id.indexOf("Ext") != -1) {
				processDiagramCanvas.drawExtUserTask(id, activityImpl.getProperty("name"), activityImpl.getX(), activityImpl.getY(), activityImpl.getWidth(), activityImpl.getHeight());
			} else {
		...
		this.activityDrawInstructions["serviceTask"] = function(){
			...
			processDiagramCanvas.setConextObject(activityImpl);
				if (id.indexOf("ext") != -1 || id.indexOf("Ext") != -1) {
				processDiagramCanvas.drawExtServiceTask(id, activityImpl.getProperty("name"), activityImpl.getX(), activityImpl.getY(), activityImpl.getWidth(), activityImpl.getHeight());
			} else {
	...
	setHighLights: function(highLights){
		...
		for (var i in highLights.finishedActivities) {
			var activityId = highLights.finishedActivities[i];
			processDiagramCanvas.highLightFinishedActivity(activityId);
		}
