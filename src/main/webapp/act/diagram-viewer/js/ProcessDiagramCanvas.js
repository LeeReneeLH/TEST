...
var FINISHED_HIGHLIGHT_COLOR = Color.LimeGreen;
//var SEQUENCEFLOW_COLOR = Color.DimGrey;
var SEQUENCEFLOW_COLOR = Color.CustomExtFlowUndo;
...
var EXT_TASK_IMAGE_PATH = "images/deployer/ext/";
var EXT_USER_TASK_IMGFILE_PRE = "type.userTask.ext.";
var EXT_SERVICE_TASK_IMGFILE_PRE = "type.serviceTask.ext.";
var IMAGE_FILENAME_EXTENSION = ".png"
var EXT_TASK_ICON_SIZE = 60;
	...
	*/
	...
	drawNoneStartEvent: function(x, y, width, height) {
	  this.g.setStart();
		...
		this.drawLabel("开始", x + 15, y + 12, width, height);
	...
	drawNoneEndEvent: function(x, y, width, height) {
	  this.g.setStart();
		...
		this.drawLabel("结束", x + 15, y + 12, width, height);
		var set = this.g.setFinish();
    this.addHandlers(set, x, y, width, height, "event");
	},
	...
	_drawNoneEndEvent: function(x, y, width, height, image, type) {
		...
		if (type == "errorEndEvent") {
			...
			fillColor = Color.CustomExtTaskUndo;
		} else 
...
// TODO: generate polylineId or do something!!
		...
		*/
        ...
        if (flowName) {
            ...
            if (contextObject.flowLabelX && contextObject.flowLabelY) {
            	textX = contextObject.flowLabelX;
            	textY = contextObject.flowLabelY;
        	}
	...
	*/
	...
	_drawExtTask: function(name, x, y, width, height, thickBorder) {
		var originalPaint = this.getPaint();
		this.setPaint(Color.CustomExtTaskUndo);
		...
		// shape
		var shape = this.g.rect(x, y, width, height, TASK_CORNER_ROUND);
		var attr = {"stroke-width": this.strokeWidth, stroke: Color.CustomExtTaskBoderUndo, fill: this.getPaint()};
		shape.attr(attr);
		//shape.attr({fill: "90-"+this.getPaint()+"-" + Color.get(250, 250, 244)});
		...
		*/
		...
		if (name) {
			...
			*/
			//console.log("draw task name: " + name);
			var boxWidth = width - (2 * TEXT_PADDING);
			var boxHeight = height -  ICON_PADDING - ICON_PADDING - MARKER_WIDTH - 2 - 2;
			var boxX = x + width/2 - boxWidth/2;
			var boxY = y + height/2 - boxHeight/2 + ICON_PADDING + ICON_PADDING - 2 - 2 + EXT_TASK_ICON_SIZE ;
			/*
			var boxWidth = width - (2 * ANNOTATION_TEXT_PADDING);
			var boxHeight = height - (2 * ANNOTATION_TEXT_PADDING);
			var boxX = x + width/2 - boxWidth/2;
	...
	*/
	...
	drawExtUserTask: function(id, name, x, y, width, height) {
		var taskImage = USERTASK_IMAGE;
		var padingX = x + ICON_PADDING;
		if (id.indexOf("ext") != -1 || id.indexOf("Ext") != -1) {
			var idArray = id.split(".");
			if (idArray.length > 1) {
				taskImage = EXT_TASK_IMAGE_PATH + EXT_USER_TASK_IMGFILE_PRE + idArray[1] + IMAGE_FILENAME_EXTENSION;
				padingX = x + width/2 - EXT_TASK_ICON_SIZE/2;
			}
		}
		
		this.g.setStart();
		this._drawExtTask(name, x, y, width, height);
		var img = this.g.image(taskImage, padingX, y + ICON_PADDING, EXT_TASK_ICON_SIZE, EXT_TASK_ICON_SIZE);
		var set = this.g.setFinish();
		this.addHandlers(set, x, y, width, height, "task");
	},
	...
	drawExtServiceTask: function(id, name, x, y, width, height) {
		
		var taskImage = SERVICETASK_IMAGE;
		var padingX = x + ICON_PADDING;
		if (id.indexOf("ext") != -1 || id.indexOf("Ext") != -1) {
			var idArray = id.split(".");
			if (idArray.length > 1) {
				taskImage = EXT_TASK_IMAGE_PATH + EXT_SERVICE_TASK_IMGFILE_PRE + idArray[1] + IMAGE_FILENAME_EXTENSION;
				padingX = x + width/2 - EXT_TASK_ICON_SIZE/2;
			}
		}
	...
	*/
	...
	highLightActivity: function(activityId){
		...
		if (activityId.indexOf("ext") != -1 || activityId.indexOf("Ext") != -1) {
			shape.attr("fill", Color.CustomExtTaskWorking);
			shape.attr("stroke", Color.CustomExtTaskBoderWorking);
		} else {
			shape.attr("stroke", HIGHLIGHT_COLOR);
		}
	},
	highLightFinishedActivity: function(activityId){
		var shape = this.g.getById(activityId);
		if (!shape) {
			console.error("Activity " + activityId + " not found");
			return;
		...
		var contextObject = shape.data("contextObject");
		if (contextObject)
			console.log("--> highLightFinishedActivity: ["+contextObject.getProperty("type")+"], activityId: " + contextObject.getId());
		else
			console.log("--> highLightFinishedActivity: ", shape, shape.data("contextObject"));
		
		shape.attr("stroke-width", THICK_TASK_BORDER_STROKE);
		
		if (activityId.indexOf("ext") != -1 || activityId.indexOf("Ext") != -1) {
			shape.attr("fill", Color.CustomExtTaskFinished);
			shape.attr("stroke", Color.CustomExtTaskBoderFinished);
		} else {
			shape.attr("stroke", HIGHLIGHT_COLOR);
		}
	},
	highLightFlow: function(flowId){
		...
		st.attr("stroke", Color.CustomExtFlowFinished);
		var withArrowHead = shapeFlow.data("withArrowHead");
		if (withArrowHead) {
			st[1].attr("fill", Color.CustomExtFlowFinished);
		}
