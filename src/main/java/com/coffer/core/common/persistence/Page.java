...
public class Page<T> {
	...
	/** 添加分页状态  0：不带模糊查询分页   1：带模糊查询分页  HZY    start **/
	...
	/** 添加分页状态  0：不带模糊查询分页   1：带模糊查询分页  HZY  end **/
	...
	public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize){
		...
		if (StringUtils.isNotBlank(searchType)){
			CookieUtils.setCookie(response, "searchType", searchType);
			this.setSearchType(Integer.parseInt(searchType));
		}
		/** 添加分页状态  0：不带模糊查询分页   1：带模糊查询分页   HZY  end **/
	}
	...
	public String toString() {
		...
		// 按回车
		sb.append("onkeydown=\"var e=window.event||this;var c=e.keyCode||e.which;if(c==13)");
		sb.append(funcName + "(this.value," + pageSize + ",'" + funcParam + "');\" ");
		// 鼠标移出
		sb.append("onblur=\"" + funcName + "(this.value," + pageSize + ",'" + funcParam + "');\"");
		sb.append("onclick=\"this.select();\"/>页");
		// 每页记录数
		sb.append("，每页<input type=\"text\" value=\"" + pageSize + "\" ");
		// 按回车
		sb.append("onkeydown=\"var e=window.event||this;var c=e.keyCode||e.which;if(c==13)");
		sb.append(funcName + "(" + pageNo + ",this.value,'" + funcParam + "');\" ");
		// 鼠标移出
		sb.append("onblur=\"" + funcName + "(" + pageNo + ",this.value,'" + funcParam + "');\" ");
		sb.append("onclick=\"this.select();\"/>条，");
		// 记录总数
