 ...
 */
package com.coffer.core.modules.act.service;
...
import com.coffer.core.modules.act.dao.ActDao;
import com.coffer.core.modules.act.entity.Act;
import com.coffer.core.modules.act.service.cmd.CreateAndTakeTransitionCmd;
import com.coffer.core.modules.act.service.cmd.JumpTaskCmd;
import com.coffer.core.modules.act.service.creator.ChainedActivitiesCreator;
import com.coffer.core.modules.act.service.creator.MultiInstanceActivityCreator;
import com.coffer.core.modules.act.service.creator.RuntimeActivityDefinitionEntityIntepreter;
import com.coffer.core.modules.act.service.creator.SimpleRuntimeActivityDefinitionEntity;
import com.coffer.core.modules.act.utils.ActUtils;
import com.coffer.core.modules.act.utils.ProcessDefCache;
import com.coffer.core.modules.act.utils.ProcessDefUtils;
...
//					List<HistoricIdentityLink> il = historyService.getHistoricIdentityLinksForProcessInstance(procInsId);
	...
	public Page<Object[]> processList(Page<Object[]> page, String category) {
	    ...
	    // 取得类型不为空的menu
	    List<Menu> menuList = UserUtils.getMenuListForCategory();
		...
		for (ProcessDefinition processDefinition : processDefinitionList) {
			// 顶级机构登录时可以查看所有流程
			if (Constant.OfficeType.ROOT.equals(UserUtils.getUser().getOffice().getType())) {
				String deploymentId = processDefinition.getDeploymentId();
				// 获取流程XML上的表单KEY
				String formKey = getFormKey(processDefinition.getId(), null);
				Deployment deployment = repositoryService.createDeploymentQuery().deploymentId(deploymentId).singleResult();
				page.getList().add(new Object[] { processDefinition, deployment, formKey });
			} else {
				for (Menu menu : menuList) {
					// 添加该用户可以访问的流程定义
					if (menu.getActCategory().equals(processDefinition.getCategory())) {
						String deploymentId = processDefinition.getDeploymentId();
						// 获取流程XML上的表单KEY
						String formKey = getFormKey(processDefinition.getId(), null);
						Deployment deployment = repositoryService.createDeploymentQuery().deploymentId(deploymentId).singleResult();
	...
	public String startProcess(String procDefKey, String businessTable, String businessId, String title, Map<String, Object> vars) {
		...
		act.setProcDefId(procIns.getProcessDefinitionId());
	 ...
	 */
	@Transactional(readOnly = false)
	public void claimAndCompleteTask(String procDefId, String procInsId, String taskKey, String userId) {
		
		Task task = taskService.createTaskQuery().includeProcessVariables().processDefinitionId(procDefId).processInstanceId(procInsId).taskDefinitionKey(taskKey).singleResult();
