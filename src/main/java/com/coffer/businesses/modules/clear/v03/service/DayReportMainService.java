...
public class DayReportMainService extends CrudService<DayReportMainDao, DayReportMain> {

	@Autowired
	private CenterAccountsMainService centerAccountsMainService;

	public DayReportMain get(String reportId) {
		return super.get(reportId);
	}

	public List<DayReportMain> findList(DayReportMain dayReportMain) {
	...
	public Page<DayReportMain> findPage(Page<DayReportMain> page, DayReportMain dayReportMain) {
	...
	public void save(DayReportMain dayReportMain) {
		super.save(dayReportMain);
	}

	@Transactional(readOnly = false)
	public void delete(DayReportMain dayReportMain) {
	 ...
	 *            主表list
	 */
	public Map<String, Object> graphicalFoldLine(List<DayReportMain> dayReportMain,
			DayReportMain alllatticePointHandin) {
		// 初始化
		...
		if (Collections3.isEmpty(dayReportMain) && list != null) {
			...
			// 将取得的折线图数据进行循环
			for (DayReportMain entitys : dayReportMain) {
				// 添加收入曲线数据
			...
			if (list != null && !Collections3.isEmpty(dataList) && !Collections3.isEmpty(dataLists)
				...
				if ("yyyy-mm-dd".equals(alllatticePointHandin.getFilterCondition())
					...
					List<DayReportMain> handInDegree = this.findChartList(dayReportMains);
					List<DayReportMain> handInDegrees = this.findChartaList(dayReportMains);
					...
					if (Collections3.isEmpty(handInDegree)) {
						...
						for (DayReportMain entity : handInDegree) {
